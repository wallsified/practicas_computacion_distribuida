...
class NodoDFS(Nodo):
    """Implementa la interfaz de Nodo para el algoritmo de DFS con reloj vectorial."""
    ...
    def __init__(self, id_nodo, vecinos, canal_entrada, canal_salida, num_nodos):
        ...
        - num_nodos = número total de nodos en la gráfica.
        - reloj = reloj vectorial (arreglo de enteros).
        - eventos = lista de eventos (envíos y recepciones).
        """
    ...
    def dfs(self, env):
        """Algoritmo DFS con reloj vectorial."""
    ...
    8.	El algoritmo termina cuando el BACK regresa al nodo distinguido y este ya no tiene ningún vecino no visitado.
        ...
        if self.distinguido:
            ...
            if self.vecinos:
                ...
                # Incrementar reloj vectorial propio antes de enviar
                self.reloj[self.id_nodo] += 1

                mensaje = ("GO", self.visitados, self.id_nodo, self.reloj.copy())

                # Registrar evento de envío (convertir set a tupla para que sea hashable)
                self.eventos.append(
                    [
                        self.reloj.copy(),
                        "E",
                        ("GO", tuple(sorted(self.visitados)), self.id_nodo),
                        self.id_nodo,
                        vecino_elegido,
                    ]
                )

                # Delay aleatorio antes de enviar
                yield env.timeout(uniform(0.1, 0.3))

                yield self.canal_salida.envia(mensaje, [vecino_elegido])
        ...
        while True:
            ...
            tipo, visitados_msg, id_emisor, reloj_recibido = mensaje

            # Actualizar reloj vectorial: max componente a componente
            for i in range(self.num_nodos):
                self.reloj[i] = max(self.reloj[i], reloj_recibido[i])

            # Incrementar reloj propio después de recibir
            self.reloj[self.id_nodo] += 1

            # Registrar evento de recepción (convertir set a tupla para que sea hashable)
            self.eventos.append(
                [
                    self.reloj.copy(),
                    "R",
                    (tipo, tuple(sorted(visitados_msg)), id_emisor),
                    id_emisor,
                    self.id_nodo,
                ]
            )
            ...
            if tipo == "GO":
                ...
                if not vecinos_no_visitados:
                    # Incrementar reloj propio antes de enviar BACK
                    self.reloj[self.id_nodo] += 1

                    msg_back = ("BACK", self.visitados, self.id_nodo, self.reloj.copy())
                    ...
                    self.eventos.append(
                        [
                            self.reloj.copy(),
                            "E",
                            ("BACK", tuple(sorted(self.visitados)), self.id_nodo),
                            self.id_nodo,
                            self.padre,
                        ]
                    )
                    ...
                    yield self.canal_salida.envia(msg_back, [self.padre])
                    self.hijos = []
                else:
                    vecino_elegido = min(vecinos_no_visitados)

                    # Incrementar reloj propio antes de enviar GO
                    self.reloj[self.id_nodo] += 1

                    msg_go = ("GO", self.visitados, self.id_nodo, self.reloj.copy())
                    ...
                    yield self.canal_salida.envia(msg_go, [vecino_elegido])
