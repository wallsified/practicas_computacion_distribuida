...
class TestPractica4:
    ...
    def verifica_orden_ascendente(self, grafica, es_vectorial):
        ...
        for nodo in grafica:
            ...
            for evento in nodo.eventos:
                reloj_actual_menor = (
                    self.compara_relojes(actual_reloj, evento[0])
                    if es_vectorial
                    else actual_reloj < evento[0]
                )
                if (
                    not reloj_actual_menor
                ):  # El reloj encontrado es menor que el que llevamos
                    return False
        ...
        for nodo in grafica:  # Iteramos cada evento...
            for evento in nodo.eventos:  # ...de cada nodo
                ...
                try:
                    ...
                    reloj_visto_menor = (
                        self.compara_relojes(reloj_visto, reloj_actual)
                        if es_vectorial
                        else reloj_visto < reloj_actual
                    )
                    reloj_visto_mayor = (
                        self.compara_relojes(reloj_actual, reloj_visto)
                        if es_vectorial
                        else reloj_visto > reloj_actual
                    )
                    envio_encontrado = (
                        tipo_visto == "E" and reloj_visto_menor and tipo_actual == "R"
                    )
                    recibo_encontrado = (
                        tipo_visto == "R" and reloj_visto_mayor and tipo_actual == "E"
                    )
    ...
    def test_ejercicio_uno(self):
        ...
        for i in range(0, len(self.adyacencias_arbol_1)):  # Gráfica 2
            grafica1.append(
                NodoBroadcast(
                    i,
                    self.adyacencias_arbol_1[i],
                    bc_pipe_1.crea_canal_de_entrada(),
                    bc_pipe_1,
                )
            )
        ...
        for i in range(0, len(self.adyacencias_arbol_2)):  # Gráfica 3
            grafica2.append(
                NodoBroadcast(
                    i,
                    self.adyacencias_arbol_2[i],
                    bc_pipe_2.crea_canal_de_entrada(),
                    bc_pipe_2,
                )
            )
    ...
    def test_ejercicio_dos(self):
        ...
        for i in range(0, len(self.adyacencias)):
            grafica.append(
                NodoDFS(
                    i, self.adyacencias[i], bc_pipe.crea_canal_de_entrada(), bc_pipe, 8
                )
            )
